    ...
    var body: some View {
        ...
        ZStack {
            ...
            ScrollView  {
                ...
                KFImage(URL(string: bookDetail.image))
                        .resizable()
                        .aspectRatio(1,contentMode: .fit)
                        .edgesIgnoringSafeArea(.top)
                
                DescriptionView(bookDescription: bookDetail)
...
struct DescriptionView: View {
    ...
    let bookDescription: BookInfo
    
    var body: some View {
        
        var bookDesc = bookDescription.title + "\n " + bookDescription.publisher + "\n " + bookDescription.author
        
        VStack (alignment: .leading) {
            //                Title
            Text(bookDescription.title)
                .font(.title)
                .fontWeight(.bold)
            //                Rating
            HStack (spacing: 4) {
...
//                        .opacity(0.6)
                ...
                VStack (alignment: .leading) {
                    Text(bookDescription.category!)
                        .font(.system(size: 16))
                        .fontWeight(.semibold)
                    Text(bookDescription.author)
                        .opacity(0.6)
                }
                .frame(maxWidth: .infinity, alignment: .leading)
            }
